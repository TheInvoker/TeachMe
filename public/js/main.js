...
function updateLocationsOnCluster(map, cluster) {
	...
	getLocations(CUR_LOCATION, function(data) {
		if (data.status == 'ok') {
			var array = data.data;
...
function updateStoresOnCluster(map, cluster) {
	...
	$.ajax({
		type: 'GET',
		url: 'https://openapi.starbucks.com/location/v1/stores?radius=10&limit=50&brandCode=SBUX&latLng=' + CUR_LOCATION.lat + '%2C' + CUR_LOCATION.lng + '&apikey=7b35m595vccu6spuuzu2rjh4',
		dataType: 'json',
		headers: {
...
function initMap() {
	...
	getCurrentLocation(function(position) {
		...
		cluster(map, "cluster-1", "cluster-2", "cluster-3", function(cluster) {
			updateLocationsOnCluster(map, cluster);
			setInterval(updateLocationsOnCluster, 60 * 1000, map, cluster);
		});

		cluster(map, "cluster-1", "cluster-2", "cluster-3", function(cluster) {
			updateStoresOnCluster(map, cluster);
			setInterval(updateStoresOnCluster, 60 * 1000, map, cluster);
		});
	});
